...
class Workload:
    ...
    def to_seller_id_and_password(self, no: int) -> (str, str):
        return "seller_{}_{}".format(
            no, self.uuid), "password_seller_{}_{}".format(no, self.uuid)

    def to_buyer_id_and_password(self, no: int) -> (str, str):
        return "buyer_{}_{}".format(no,
                                    self.uuid), "buyer_seller_{}_{}".format(
                                        no, self.uuid)
    ...
    def update_stat(self, n_new_order, n_payment, n_cancel_order, n_send_order,
                    n_receive_order, n_new_order_ok, n_payment_ok,
                    n_cancel_order_ok, n_send_order_ok, n_receive_order_ok,
                    time_new_order, time_payment, time_cancel_order,
                    time_send_order, time_receive_order):
        # 获取当前并发数
        ...
        if (self.n_payment != 0 and self.n_new_order != 0
            ...
            logging.info(
                ...
                "R=OK:{} Thread_num:{} TOTAL:{} LATENCY:{}".format(
                    ...
                    self.n_payment,
                    self.time_payment /
                    self.n_payment,  # 付款延迟:(付款所用时间/并发数)/付款订单数
                    self.n_cancel_order_ok,
                    n_cancel_order_diff,
                    self.n_cancel_order,
                    self.time_cancel_order /
                    self.n_cancel_order,  # 取消订单延迟:(取消所用时间/并发数)/取消订单数
                    self.n_send_order_ok,
                    n_send_order_diff,
                    self.n_send_order,
                    self.time_send_order /
                    self.n_send_order,  # 订单送货延迟:(状态改变所用时间/并发数)/寄送订单数
                    self.n_receive_order_ok,
                    n_receive_order_diff,
                    self.n_receive_order,
                    self.time_receive_order /
                    self.n_receive_order,  # 订单收货延迟:(状态改变所用时间/并发数)/订单收货数
                ))
        self.lock.release()
        # 旧值更新为新值，便于下一轮计算
