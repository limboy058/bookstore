...
class TestSearchBook:
    ...
    def test_search_book_id(self):
        for b in self.books:
            ...
            #test id
            book_id=b.id
            code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,id=book_id)
            assert code==200
            assert(len(lst)>0)
            for i in lst:
                res=json.loads(i)
    ...
    def test_search_book_title(self):
        ...
        for b in self.books:
            ...
            #test title
            foozytitle=str(cnt)
            code,lst,msg = self.auth.searchbook(page_no=0,page_size=self.test_books_num,foozytitle=foozytitle)
    ...
    def test_search_book_author(self):
        for b in self.books:
            ...
            author=b.author
            if(author!=None):
                code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,author=author)
                assert code==200
                assert(len(lst)>0)
                for i in lst:
                    res=json.loads(i)
    ...
    def test_search_book_tags(self):
        for b in self.books:
            ...
            while(len(reqtags)>2):
                reqtags.pop()
            code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,reqtags=reqtags)
            assert code==200
            assert(len(lst)>0)
            for i in lst:
                res=json.loads(i)
    ...
    def test_search_book_isbn(self):
        for b in self.books:
            ...
            isbn=b.isbn
            if(isbn !=None):
                code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,isbn=isbn)
                assert code==200
                assert(len(lst)>0)
                for i in lst:
                    res=json.loads(i)
    ...
    def test_search_book_price(self):
        for b in self.books:
            ...
            code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,lowest_price=lowest_price)
            assert code==200
            assert(len(lst)>0)
            for i in lst:
                res=json.loads(i)
            ...
            code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,highest_price=highest_price)
            assert code==200
            assert(len(lst)>0)
            for i in lst:
                res=json.loads(i)
    ...
    def test_search_book_pub_year(self):
        for b in self.books:
            ...
            if(b.pub_year!=None):
                ...
                code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,lowest_pub_year=lowest_year)
                assert code==200
                assert(len(lst)>0)
                for i in lst:
                    res=json.loads(i)
                ...
                code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,highest_pub_year=highest_year)
                assert code==200
                assert(len(lst)>0)
                for i in lst:
                    res=json.loads(i)
    ...
    def test_search_book_store_id(self):
        for b in self.books:
            #test store_id
            code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,store_id=self.store_id)
            assert code==200
            assert(len(lst)>0)
            for i in lst:
                res=json.loads(i)
        ...
        for b in self.books:
            ...
            if(publisher!=None):
                if(publisher is not None):
                    code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,publisher=publisher)
                    assert code==200
                    assert(len(lst)>0)
                    for i in lst:
                        res=json.loads(i)
    ...
    def test_search_book_translator(self):
        for b in self.books:
            ...
            if(translator!=None):
                code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,translator=translator)
                assert code==200
                assert(len(lst)>0)
                for i in lst:
                    res=json.loads(i)
    ...
    def test_search_book_binding(self):
        for b in self.books:
            ...
            binding=b.binding
            if(binding!=None):
                code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,binding=binding)
                assert code==200
                assert(len(lst)>0)
                for i in lst:
                    res=json.loads(i)
    ...
    def test_search_book_stock(self):
        for b in self.books:
            #test having_stock
            code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,having_stock=True,store_id=self.store_id)
            assert code==200
    ...
    def test_search_book_non_exist_store(self):
        for b in self.books:
            ...
            code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,store_id=tmp_store_id)
            assert code==200
            assert(len(lst)==0)
    ...
    def test_search_book_with_order(self):
        for b in self.books:
            #test order
            code,lst,_ = self.auth.searchbook(page_no=0,page_size=self.test_books_num,order_by_method=['stock_level',1])
            assert code==200
